...
namespace puzzle {
	...
	public:
		...
		Sudoku(const int level) : mLevel(level) {
			mValueList.clear();
			for (char c = '1'; c <= '0' + mSize; c++) mValueList.emplace_back(c);
		}
	...
	private:
		void resetSudoku() {
			...
			for (int i = 0; i < mSize; i++) mSudokuSolved[i] = string(mSize, blank());
		}
		...
		bool canSetValue(const sudoku_t& sudoku, int i, int j, char val) const {
			for (int k = 0; k < mSize; k++) if (sudoku[i][k] == val) return false;
			for (int k = 0; k < mSize; k++) if (sudoku[k][j] == val) return false;
			...
			for (int y = 0; y < mSize; y++) {
				for (int x = 0; x < mSize; x++) {
					...
					for (char c : mValueList) {
						if (!canSetValue(sudoku, y, x, c)) continue;

						sudoku[y][x] = c;
						if (isSingleSolution(sudoku, count)) count++;
